...
for (let attempt = 1; ; attempt++) {
  ...
  try {
    Bun.serve({
      port: PORT,
      hostname: HOST,
      // Default 10s kills long-running requests. The very first DB-backed
      // request applies the schema (a fresh Neon apply takes >10s) and Neon
      // HTTP calls can be slow on cold starts, so allow generous idle time.
      idleTimeout: 120,
      async fetch(req) {
